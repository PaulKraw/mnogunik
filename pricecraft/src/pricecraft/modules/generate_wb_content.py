...
def main():
    ...
    try:
        ...
        try:
            ws = sh.worksheet("content_wb")
        except Exception:
            ws = sh.add_worksheet(title="content_wb", rows=100, cols=10)
...
# 10. Запускаем PHP скрипт генерации через HTTP
 ...
 # 11. Ожидаем появление файла
        ...
        try:
            ...
            TARGET_COLS = ['Description_wb', 'ImageUrls_wb']  # куда вставляем в content
            ...
            if df_current.empty or 'articul' not in df_current.columns:
                ...
                base_columns = ['articul', 'Description_wwb', 'ImageUrls_wb']
                ...
                for articul, data in file_data_dict.items():
                    row = {
                        'articul': articul,
                        'Description_wb': data.get('Description', ''),
                        'ImageUrls_wb': data.get('ImageUrls', '')
                    }
                    rows.append(row)
                ...
                for articul, file_data in file_data_dict.items():
                    if articul in content_dict:
                        ...
                        for idx in indices:
                            df_current.at[idx, 'Description_wb'] = file_data.get('Description', '')
                            df_current.at[idx, 'ImageUrls_wb'] = file_data.get('ImageUrls', '')
                            updated_count += 1
                        if len(indices) > 1:
                ...
                # 7. Очищаем wb поля у артикулов которых нет в файле
                cleared_count = 0
                for idx, row in df_current.iterrows():
                    articul = str(row['articul']).strip() if 'articul' in row and pd.notna(row['articul']) else ''
                    if articul and articul not in file_data_dict:
                        df_current.at[idx, 'Description_wb'] = ''
                        df_current.at[idx, 'ImageUrls_wb'] = ''
                        cleared_count += 1
            ...
            for col in ['Description_wb', 'ImageUrls_wb']:
                if col in cols:
                    cols.remove(col)

            # Вставляем их на позиции 1 и 2 (индексы 1 и 2)
            cols.insert(1, 'Description_wb')
            cols.insert(2, 'ImageUrls_wb')
    ...
    # print(f"[generate_wb_file] Выполнено в {datetime.datetime.now()}")
        get_config.finish("generate_wb_content")

        write_status("finished", "контент создан", "generate_wb_content")

        write_log(f"[generate_wb_content] Выполнено в {datetime.now()}")
    
    except Exception as e:
        tb = traceback.format_exc()
        write_log("ERROR: " + str(e))
        write_log(tb)
        write_status("error", str(e), "generate_wb_content")
